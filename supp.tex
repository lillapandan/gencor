...
and if $\beta$ and $\gamma$ are reasonably polygenic. s
...
We provide this interpretation in terms of the heritability estimators; the corresponding statements for the genetic covariance estimators
...
\end{equation}
where $\hat{\ell}_j$ denotes the sample LD Score for SNP $j$, and $a$ is a term that depends on sample structure.
This approximation justifies replacing $\hat{\ell}_j$ and $\lbar_X$ with an unbiased estimate of population LD Score 
($\ell_j$) from a sequenced reference panel.

This approximation step provide several advantages:
\begin{enumerate}
	...
	$\mathscr{O}(MN)$ for computing summary statistics, and time $O(M_g)$ for the regression. 
...
\input{./table/qt_cc_sim_table}\ref{asdf}
