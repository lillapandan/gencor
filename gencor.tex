...
A recent focus in statistical genetics has been combining genetic association results from multiple phenotypes simultaneously in order understand the relationship between them.
Here we describe a novel method for estimating genetic correlations between traits that requires only summary statistics from genome-wide association studies (GWAS) as input.
We apply our method to 25 sets of publicly available GWAS summary statistics, which include more than 1.5 million genotype-phenotype pairs. 
...
In addition, we report several novel results, including a positive genetic correlation between anorexia nervosa (AN) and schizophrenia and a negative genetic correlation between AN and body mass. 
...
We provide open source software implementing these methods.
...
The earliest attempts to probe for traits with genetic overlap were twin and family studies \cite{vandenberg1965multivariate, 
kempthorne1961interpretation,
loehlin1966genetic,
neale1992methodology,
lichtenstein2009common}.
These methods have been applied to a wide spectrum of traits,
though family methods have the disadvantage of requiring measurements of different traits on the same individuals.
...
etiology using these effect size estimates, which circumvents the requirement to measure the same traits on the same individuals.
This can substantially reduce the cost and difficult of epidemiological studies for phenotypes that are rare or expensive to assay.

One widely-used technique for testing relationships between phenotypes using GWAS data is Mendelian randomization  
\cite{smith2003mendelian, smith2014mendelian}, which is the specialization to genetics of the more general instrumental variables technique \cite{angrist2008mostly}
and has proved effective for traits for which there exist large-effect genetic variants \cite{voight2012plasma,do2013common}. 
...
For such traits, Mendelian randomization implemented with a small number of SNPs as instruments suffers from low power and weak instrument bias \cite{angrist2008mostly}.

In this paper, we  estimate genetic correlation, a quantity whose definition includes the effects of all SNPs, including those that do not reach genome-wide significance (Methods).
However, in cases where Mendelian randomization is applicable,
genetic correlation can be interpreted as a re-scaling of the instrumental variables estimate obtained from an instrument constructed using all SNPs (see Methods).
The two main techniques for estimating genetic correlation from GWAS data are restricted maximum likelihood (REML) \cite{yang2010, yang2011gcta, lee2012estimation, pgccdg2013, vattikuti2012heritability, chen2014estimation}
...
Here, we introduce a method for estimating genetic correlation that requires only GWAS summary statistics as input, is not biased by sample overlap
and is several orders of magnitude less computationally demanding than REML.
We apply this method to a large set of GWAS for which the summary statistics have been made publicly available for download and report genetic correlations between 300 pairs of traits. 
...
In addition, we report several novel results, including a positive genetic correlation between anorexia nervosa (AN) and schizophrenia and a negative genetic correlation between AN and BMI. 
These results demonstrate the advantages of a polygenic modeling framework, since there are currently no SNPs associated with AN at genome-wide significance \cite{boraska2014genome}. 
...
Our method for estimating genetic correlation from summary statistics relies on the fact that the $\chi^2$-statistic at a given SNP incorporates the effects of all SNPs in LD with that SNP\cite{yang2011genomic,buliksullivan2014}. 
...
A similar relationship holds if we replace $\chi^2$-statistics for a single study with the product $z_1z_2$, 
where $z_i$ denotes the $Z$-score for study $i$.
...
\end{equation}
where $N_i$ is the sample size for study $i$, $\rho_g$ is genetic covariance (defined in methods), $\ell_j$ is LD Score, $N_s$ is the number of samples shared between study 1 and study 2, and $\rho$ is the phenotypic correlation among the $N_s$ overlapping samples.
If study one and study two are the same study, then this reduces to the single-phenotype result from \cite{buliksullivan2014}, because the genetic covariance between a trait and itself is simply $h^2$, and $\chi^2 = z^2$.
...
As a consequence of equation \ref{reg_eqn}, we can estimate genetic covariance using the slope from the regression of $z_{1j}z_{2j}$ on LD Score. 
This estimate will not be biased even if the two studies share samples, because sample overlap only 
...
Similarly, the estimate will be only minimally biased by shared population stratification or genotpying artifacts, because these sources of bias are also not correlated with LD Score and will
only affect the intercept \cite{buliksullivan2014}.
If we can rule out shared population stratification,
 and the intercept term $\rho N_s/\sqrt{N_1N_2}$ is known ahead of time (\emph{e.g.,} if there is no sample overlap and $N_s=0$), 
we can obtain a lower standard error by constraining the intercept to equal this value.

If we normalize genetic covariance to lie in the interval $[-1,1]$, we obtain genetic correlation: 
$r_g := \rho_g/\sqrt{h^2_1h^2_2},$ where $h^2_i$ denotes the SNP-heritability of trait $i$. 
...
samples used for each disease were non-overlapping, we also estimated genetic correlation using LD Score regression with constrained intercept.
...
highly correlated anthropometric phenotypes are displayed in supplementary table XXX, 
...
Genetic correlations that are significantly different from zero at significance level 0.05 after Bonferroni correction for 300 tests are given an asterisk. 
...
Our estimates of genetic correlations among metabolic traits are generally consistent with the estimates obtained using REML in Vattikuti \emph{et. al.}
...
Our results on metabolic traits are directionally consistent with the recent Mendelian randomization results from \cite{wuertz2014metabolic}.
...
Type & Phenotype 1 & Phenotype 2 & $r_g$ (se) & $p$-value \\
\hline
\multirow{15}{*}{Epidemiological} 
& Age at Menarche & Height & 0.11 (0.03) & $6\times10^{-5\ **}$  \\
& Age at Menarche & T2D & -0.13 (0.04) & $3\times10^{-3}$  \\
& Age at Menarche & CAD & -0.11 (0.05) & $4\times10^{-2}$ \\
& Age at Menarche & Triglycerides & -0.15 (0.04) & $1\times10^{-3\ *}$ \\
& Alzheimer's & College & -0.30 (0.08) & $1\times10^{-4\ **}$ \\
& Bipolar Disorder & College & 0.026 (0.064) & $6\times10^{-5\ **}$ \\
& Obesity & College & -0.23 (0.04) & $2\times10^{-8\ **}$\\
& CAD & College & -0.278 (0.07) & $1\times10^{-4\ **}$ \\
& Triglycerides & College &-0.30 (0.04)&$5\times10^{-12\ **}$ \\
& Anorexia Nervosa & Obesity & -0.20 (0.04) & $4\times10^{-6\ **}$ \\
& Schizophrenia & Crohn's & 0.013 (0.04) & $3\times10^{-3\ }$\\ 
& Schizophrenia & Ulcerative Colitis & 0.012 (0.05) & $1\times10^{-2}$ \\
& Ever/Never Smoker & College & -0.39 (0.07) & $1\times10^{-9\ **}$ \\
& Ever/Never Smoker & Obesity & 0.22 (0.05) & $7\times10^{-5\ **}$ \\
& Height & CAD & -0.17 (0.05) & $2\times10^{-4\ *}$\\
\hline
\multirow{3}{*}{Novel} 
& ASD & College & 0.28 (0.08) & $5\times10^{-4\ *}$\\
...
& Schizophrenia & LDL & -0.02 (0.03) & 0.64 \\
& Schizophrenia & HDL & 0.03 (0.04) & 0.50 \\
& Crohn's & Rheumatoid Arthritis & -0.02 (0.09) & 0.83 \\
...
\small{\textit{\label{table:results}Genetic correlation estimates, standard errors and $p$-values for selected pairs of traits. Results are grouped into
 genetic correlations that are consistent with established epidemiological associations (``Epidemiological''), novel genetic correlations (``Novel''), and interesting null results (``Null''). 
Results that pass paper-wide multiple testing correction at 1\% FDR are given a single asterisk; results that pass paper-wide Bonferroni correction at significance level 0.05 are given two asterisks.
For completeness, we present some genetic correlations that are directionally consistent with epidemiological associations but that do not pass multiple testing correction in our data. Genetic correlations between Alzheimer's and psychiatric diseases other than schizophrenia are also close to zero, but with larger standard error, due to the smaller sample sizes for the other psychiatric diseases.}}}
\end{table}

The first row of table \ref{table:results} lists genetic correlation results that are consistent with well-known epidemiological association, but, to the best of our knowledge, 
have never been reported using genetic data.
...
We estimate a statistically significant negative genetic correlation between educational attainment and Alzheimer's disease, 
...
The positive genetic correlation between smoking and obesity is in the opposite direction of the epidemiological association (smoking is associated with lower BMI, although smoking cessation 
...
Last, the negative genetic correlation between height and coronary artery disease (possibly related to the negative genetic correlation between height and LDL, a known risk factor for coronary artery disease \cite{do2013common, burgess2014using}) is consistent with a replicated epidemiological association  \cite{wang2011associations,hebert1993height,rich1995height}.

% Novel results

The second row of table \ref{table:results} lists results that are, to the best of our knowledge, novel.
...
however, our result is more statistically significant.
...
(a proxy for cognitive function \cite{deary2007intelligence, calvin2010sex, rietveld2013gwas}, at least after controlling for socioeconomic status).
...
Finally, the third row of table \ref{table:results} lists several instances where the genetic correlation is close to zero, in contrast to previous reports of epidemiological correlation or genetic overlap.
...
Our estimate of zero genetic correlation between schizophrenia and plasma lipids or cardiovascular disease contrasts with earlier reports of extensive pleiotropy between schizophrenia and triglycerides \cite{andreassen2013improved2}. 
However, the observation from Andreassen, \emph{et. al.} \cite{andreassen2013improved2} could potentially be explained the sensitivity of the method used to the properties of a few regions with high linkage disequilibrium, 
rather than trait biology (Supplementary Table XXX). 
...
This is an example of pleiotropy without genetic correlation (see Methods for a discussion of the distinction).
...
but could also result from non-causal shared genetic etiology, perhaps mediated by the genetic correlation of -0.55 between HDL and triglycerides (TG). 
...
We have developed a method that allows us to rapidly screen hundreds of pairs of phenotypes for associations without having to measure all phenotypes on all individuals.
...
\end{equation}
where the maximization is performed in the population (\emph{i.e.,} in the infinite data limit), rather than in a finite sample.
