...
or linkage disequilibrium, as discussed in \cite{speed2012improved}; 
...
Results from these simulations are presented (in order) in supplementary tables \ref{supp-parallel}, \ref{supp-antiparallel} and
\ref{supp-depcor}.
...
\cite{golan2013narrowing}, and so it is not surprising that LD Score regression performs about as well.
\begin{figure}[!ht]
...
and the black lines indicate one standard error. 
...
We applied our method to a large set of publicly available summary statistics, 
...
smoking \cite{tobacco2010genome},
...
LD Score regression heritability estimates are biased downwards by genomic control correction 
...
that appear in 1000 Genomes, denoted $h^2(5\myhyphen50\%)$ (see Methods).
This quantity is not in general the same as the quantity obtained from GCTA-REML described 
as the ``variance explained by genotyped SNPs'', and denoted either $h^2_g$ or $h^2_{SNP}$ 
(indeed, different $h^2_g$ estimates from different studies are usually not directly comparable to each other, 
because they depend on the set of SNPs used for computing the kinship matrix). 

%%% Discuss h2(5-50%)
The full list of 700 genetic correlation estimates are provided in tabular format in supplementary table \textbf{NNN},
and are displayed as a heatmap in table \textbf{NNN}. 
...
Nevertheless, we do observe some interesting individual results (table \textbf{NNN}).
A few examples:
We estimate genetic correlations close to zero between Alzheimer's and all of the psychiatric traits. 
Instead, Alzheimer's clusters (weakly, but significantly) with the anthopometric traits.
