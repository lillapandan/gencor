...
Here, we introduce a computationally trivial method for estimating genetic correlation that requires only GWAS summary statistics and is not biased by sample overlap.
We apply this method to data from 25 GWAS and report genetic correlations between 300 pairs of phenotypes. 
Our results confirm many previously reported genetic overlaps and epidemiological associations, 
thus validating the utility of genetic correlation as an epidemiological tool.
...
where $z_{i,j}$ denotes the $Z$-score for study $i$ and SNP $j$.
...
\end{equation}
where $N_i$ is the sample size for study $i$, $\rho_g$ is genetic covariance (defined in methods), $\ell_j$ is LD Score, $N_s$ is the number of samples shared between study 1 and study 2, and $\rho$ is the phenotypic correlation among the $N_s$ overlapping samples (Supplementary Note).
...
A similar relationship holds if one or both of the studies is an ascertained study of a binary phenotype (Supplementary Note), in which case the estimate of genetic covariance will be on the observed scale.
...
This regression does not appear to be sensitive to modeling assumptions about genetic architecture (Methods)
...
Similarly, the estimate of genetic covariance will be only minimally biased by shared population stratification or genotpying artifacts, 
because these sources of bias are also not correlated with LD Score and will only affect the intercept \cite{buliksullivan2014}.
...
\cite{vattikuti2012heritability} (Supplementary Table \ref{supp-vattikuti}). 
...
& Schizophrenia & Crohn's Disease & 0.013 (0.04) & $3\times10^{-3\ }$\\ 
...
\hline
\multirow{11}{*}{Null} 
& Anorexia Nervosa & Alzheimer's  & 0.09 (0.09) & 0.32\\
& Major Depression &Alzheimer's & 0.08 (0.15) & 0.60 \\
& Bipolar Disorder & Alzheimer's &  -0.021 (0.09) & 0.82 \\
& Autism Spectrum Disorder & Alzheimer's & 0.12 (0.12) & 0.30 \\
...
\small{\textit{\label{table:results}Genetic correlation estimates, standard errors and $p$-values for selected pairs of traits. Results are grouped into
 genetic correlations that are novel genetic results, but are consistent with established epidemiological associations (``Epidemiological''), novel genetic correlations (``Novel'') and interesting null results (``Null''). 
 The $p$-values displayed are uncorrected $p$-values.
Results that pass paper-wide multiple testing correction at 1\% FDR are given a single asterisk; results that pass Bonferroni correction at significance level 0.05 after correction for the 300 tests in figure \ref{Fig:300 Gencors} are given two asterisks.
...
although we emphasize that the evidence for genetic correlation is not yet conclusive.
...
%%% what to say here???
\textbf{[waiting on advice from Mark Daly about what to say here]}.
...
We estimate near-zero genetic correlations between Alzheimer's disease and five psychiatric disorders. 
This is notable given the high genetic correlations among the five psychiatric disorders, 
and suggests that the genetic basis of Alzheimer?s disease is distinct from these other conditions. 
...
Although it may be possible to extend existing methods such as LD-pruned polygenic scores to work on pairs of summary statistics (F. Dudbridge, personal communication), 
such methods would not be biased from sample overlap, and would suffer from the disadvantage that LD-pruning always results in loss of information.
...
Although genetic correlation is immune to environmental confounding, genetic correlation is still subject to genetic confounding, which is normally called confounding via pleiotropy 
in the Mendelian randomization literature. 
...
First, LD Score regression requires large sample sizes in order to give estimates with acceptable standard error 
...
Methods such as REML that use individual genotypes are more statistically efficient.
Second, LD Score regression assumes that all individuals in the GWAS were sampled from populations with similar LD Scores,
and that these LD Scores can be estimated from sequenced reference data such as 1000 Genomes 
\cite{10002012integrated}.
...
This means that the non-pleiotropic SNPs required for classical Mendelian randomization
will generally be hard to find, which highlights the importance of methods that account naturally for pleiotropy \cite{do2013common, burgess2014using},
