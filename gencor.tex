...
and genetic correlation from methods such as LD Score regression and GCTA-REML. 
...
For comparison, GCTA-REML takes time $\mathscr{O}(MN^2)$ for computing the genetic relatedness matrix (GRM)
...
GCTA-REML with a GRM computed using only $\sim\!\!1$ million genotyped SNPs 
estimates a different quantity than LD Score regression with sum $r^2$
taken over all $\sim\!\!15$ million SNPs in 1kG (1kG LD Scores).
GCTA-REML with $\sim\!\!1$ million genotyped SNPs estimates quantity which describe properties of 
best linear predictor of phenotype that one could obtain using only the $\sim\!\!1$ million genotyped SNPs.
LD Score regression with 1kG LD Scores estimates quantities which are properties of the best linear predictor of phenotype
that one could obtain using all 1kG SNPs.
...
Thus, to give a fair comparison to GCTA-REML, we include results from LD Score regression results obtained using
an LD Score with the sum of $r^2$ taken only over the 1.2 million autosomal HapMap 3 (HM3) SNPs \cite{international2010integrating} -- which we refer to as HM3 LD Score -- as well as results from LD Score regression with
a 1kG LD Score partitioned on derived allele frequency (DAF) and recombination rate, to account for potential DAF-
...
as well as sample overlap, but at the cost of a substantial increase in standard error, which is non-negligible at the relatively
small sample sizes from \cite{cross2013identification}.
...
The genetic correlation estimates from LD Score regression with HM3 LD Scores were very similar to the results from GCTA-REML. LD Score regression without intercept gave standard errors that were only slightly larger than GCTA-REML,
...
(\emph{e.g.,} ADD, Autism).
The results from the DAF- and recombination rate-partitioned LD Score regression with 1kG LD Scores ... WRITE ME
...
estimates obtained from GCTA-REML in \cite{pgccdg2013}.
The horizontal axis indicates pairs of phenotypes, and the vertical axis indicates genetic correlation.
Colors indicate different estimation procedures, as described in the main text.
