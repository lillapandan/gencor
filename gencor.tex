...
Put another way, the 
natural definition for ``observed scale genetic correlation'' turns out to be the same as regular genetic correlation. This is convenient: we can compute genetic correlations for binary traits on a sensible
...
In a typical GWAS setting, the regression SNPs are predominately common SNPs, so multiplying the slope from LD Score regression by $M$ (which includes rare SNPs) amounts to extrapolating that $h^2$ per SNP
among common variants is the same as $h^2$ per SNP among rare variants.
This is a particularly risk form of extrapolation, because there are many more rare SNPs than common SNPs.

It is probably reasonable to treat $h^2$ per SNP as a roughly constant function of MAF for SNPs with MAF above 5\%, but we have very little information about $h^2$ per SNP for SNPs with MAF below 5\%.
Therefore we report $h^2_{5\myhyphen50\%}$ to avoid excessive extrapolation error.
...
genetic covariance parameter estimated by GCTA is similar to the relationship between 
