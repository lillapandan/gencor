...
Table XXX shows LD Score regression estimates and standard errors from 1000 simulations of quantitative traits with complete sample overlap.
These simulations confirm that LD Score regression is not biased by sample overlap and that the block jackknife 
standard errors give correct inference (Methods).
Table SN shows results from simulations with one quantitative trait and one ascertained binary trait, 
and table SN+1 shows results from simulations with two ascertained binary traits.
...
We obtain approximately correct estimates of genetic correlation even in simulations with models of genetic architecture where our estimates of heritability and genetic covariance are biased (Tables \ref{parallel}).
...
If we know the amount of sample overlap ahead of time, we can reduce the standard error by constraining the intercept with the \texttt{--constrain-intercept} flag in \texttt{ldsc}.
...
\cite{schizophrenia2014biological}.
