...
however, the interpretation of results from these studies can be confounded by environmental factors and reverse causation. 
...
Methods based on GWAS data have the particular advantage that because GWAS yields effect-size estimates for individual genetic variants,
it is not necessary to measure multiple traits on the same individuals.
This is a sizable advantage for phenotypes that are rare or costly to assay.  
There are three main approaches for discovering directed relationships between phenotypes using GWAS data. 
...
Our method for estimating genetic correlation from summary statistics relies on the fact that the $\chi^2$-statistic at a given SNP incorporates the effects of all SNPs in LD with that SNP\cite{yang2011ejhg,buliksullivan2014}. 
...
samples used for each disease were non-overlapping, applied also fit LD Score regression with no intercept.
...
We used LD Score regression to estimate genetic correlations between 25 phenotypes (Table \ref{data}, URLs, Methods), 
