...
Estimating the genetic correlation can lead to insight about the shared genetic basis of two phenotypes. 
For example, the genetic correlation between schizophrenia and bipolar disorder is estimated to be 0.8 \cite{BipSczGC}, 
indicating that despite the different clinical presentation of these two disorders, there is a strong shared genetic basis.  
[[Did the high gc between UC and CD lead people to do things differently like pool samples or something like that?
It would be great to have one or two examples of why genetic correlation is interesting here.]]
...
usually estimated from samples of at most a few tens of thousands of individuals,
...
Here, we propose a modification of LD Score regression \cite{buliksullivan2014}
that can estimate the genetic correlation between two traits from GWAS summary statistics, 
allowing us to estimate hundreds of genetic correlations with computational ease. 
These estimates greatly extend the understanding gleaned from the handful of previously estimated
...
i.e.\ . $\ell_j = \sum_k r^2(j,k)$.
Then assuming a simple model of genetic architecture where for each phenotype, SNP effect sizes are drawn in an uncorrelated fashion from distributions with mean zero and a fixed variance, we have
\begin{equation}
\label{eq.main}
...
is an ascertained study of a binary phenotype, and so the same method can be used regardless of whether the $Z$-scores are from a study of quantitative or case-control traits. If effect sizes do not share a variance, but instead have a variance that depends on minor allele frequency (MAF) or linkage disequilibrium (LD), as discussed in \cite{speed2012improved}, we can use partitioned LD Score regression as described in the results and methods sections of this paper and also \cite{finucane2014partitioning}. Equation~\eqref{eq.main} and these analogues are derived in the supplementary note. 
...
Instead, Alzheimer's disease clusters (weakly, but significantly) with anthropometric and metabolic traits.
...
demonstrate that GCTA-REML can be confounded by MAF- or LD-dependent genetic architectures.
...
We used simulations to explore the behavior of both  partitioned and naive (i.e.\ not partitioned) LD Score regression 
...
The heritability and genetic covariance estimates from naive LD Score regression were badly biased in all cases
...
With LD Score regression, we can model the effects of SNPs that are not genotyped in the GWAS
because we have LD information about these SNPs from a sequenced reference panel.
(See the section ``LD Score Regression is [[approximates?]] Haseman-Elston Regression'' in the Supplementary Note.) 
In particular, we usually use LD Scores with $r^2$s summed over all 1000 Genomes SNPs that are polymorphic in Europeans \cite{10002012integrated}, 
corresponding to an assumption that any of these SNPs could be contributing to heritability. We will call these 1000G LD Scores.

GCTA-REML, on the other hand, is based on a model that only includes the effects of genotyped SNPs. 
So for our comparison, we temporarily assumed an analogous model in which only the 1.2 million autosomal HapMap 3 SNPs  \cite{international2010integrating} were causal, 
which meant using LD Scores with  
the sum of $r^2$s taken only over the HapMap 3 SNPs
(hereafter referred to as HapMap3 LD Score). 

While here we use non-partitioned HapMap3 LD Scores for our comparison, 
in general we believe that partitioned 1000G LD Scores provide more reliable results.
The genetic correlation estimates in the next section (``Application to a Large Set of Publicly Available Summary Statistics")
use partitioned 1000G LD Scores and much larger sample sizes, and so we believe those results to be more reliable.
...
We applied our method to 23 [[I think this number is wrong]] publicly available sets of GWAS summary statistics,
including 
...
These are technically estimates of the heritability accounted for by SNPs with 5-50\% MAF 
...
standard error. 
For smaller sample sizes, GCTA-REML is a more statistically efficient estimator of genetic correlation when its assumptions
