...
\cite{yang2010, yang2011gcta} -- hereafter referred to as REML -- or polgyenic risk scores \cite{dudbridge2013power},
...
Thus, to give a fair comparison to REML, we include results from LD Score regression results obtained using
...
This suggests that when estimating genetic correlation with small datasets or phenotypes with low heritability -- that is, in
situations where the mean square error (MSE) is dominated by variance rather than bias --  
...
This quantity is not in general the same as the quantity estimated by REML described 
