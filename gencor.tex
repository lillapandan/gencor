...
Discovering directed relationships between phenotypes is a fundamental goal of epidemiology, 
with applications to classification and treatment of disease, as well as to development of pharmaceutical drugs.  
The traditional strategy in epidemiology is to search for correlations between phenotypes via cross-sectional or longitudinal observational studies; 
however, the interpretation of results from these studies can be confounded and are vulnerable to reverse causation. 
An alternative strategy that is effective for heritable traits and is more robust to confounding is to search instead for pairs of phenotypes with shared genetic etiology \cite{smith2003mendelian}. 

The earliest attempts to probe for traits with genetic overlap were twin and family studies.
The applicability of these methods is limited, because they require sampling twins and families and measuring different traits on the same individuals.
...
which  has proved effective for traits for which there exist large-effect genetic variants \cite{smith2014mendelian}. 
...
For such traits, Mendelian randomization suffers from low power and weak instrument bias \cite{angrist2008mostly}.

When few large-effect SNPs are available, it is more effective to use techniques such as restricted maximum likelihood (REML) \cite{yang2010, yang2011gcta, lee2012estimation}
...
These methods report estimates of a parameter called genetic correlation, which is the framework that we adopt in this paper; however,
...
We apply this method to all GWAS for which the summary statistics have been made publicly available for download (to our knowledge) and report genetic correlations between hundreds of pairs of traits. 
...
If we can rule out shared population stratification,
 and the intercept term $\rho N_s/\sqrt{N_1N_2}$ is known ahead of time (\emph{e.g.,} if there is no sample overlap and it is zero), 
we can obtain a lower standard error by constraining the intercept.
...
We used LD Score regression to estimate genetic correlations among 25 phenotypes (Table \ref{data}, URLs, Methods).
...
Our estimates of genetic correlations among metabolic traits are generally consistent with the estimates obtained using REML in 
...
Some of these results have also been reported in the twin study literature; for example, our estimate of 0.12 (0.03) ($p=5\times10^{-4}$) for the genetic correlation between adult height and educational attainment is consistent with the genetic correlation estimates from \cite{keller2013genetic} of 0.08 in males and 0.17 in females between adult height and IQ (which is highly correlated with educational attainment \cite{deary2007intelligence, calvin2010sex, rietveld2013gwas}). 
...
Third, we estimate a genetic correlation of 0.28 (0.08) ($p=5\times10^{-4}$) between autism spectrum disorder (ASD) and educational attainment (a proxy for cognitive function \cite{deary2007intelligence, calvin2010sex, rietveld2013gwas}).
...
However, the observation from \cite{andreassen2013improved2} could potentially be explained the sensitivity of the method used to the properties of a few regions with high linkage disequilibrium, 
...
it appears that the directions of effect are in the opposite direction as often as they are concordant, yielding close to zero genetic correlation overall.
